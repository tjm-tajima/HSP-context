...
| [mes](#mes) | メッセージ表示 |
| [print](#print) | メッセージ表示 |
| [title](#title) | タイトルバー設定 |
| [dialog](#dialog) | ダイアログを開く |
| [bgscr](#bgscr) | 枠のないウィンドウを初期化 |
| [bmpsave](#bmpsave) | 画面イメージセーブ |
| [boxf](#boxf) | 矩形を塗りつぶす |
| [buffer](#buffer) | 仮想画面を初期化 |
| [chgdisp](#chgdisp) | 画像解像度を変更する |
| [color](#color) | カラー設定 |
| [font](#font) | フォント設定 |
| [gcopy](#gcopy) | 画面コピー |
| [gmode](#gmode) | 画面コピーモード設定 |
| [gsel](#gsel) | 描画先指定、ウィンドウ最前面、非表示設定 |
| [gzoom](#gzoom) | 変倍して画面コピー |
| [palcolor](#palcolor) | 描画パレット設定 |
| [palette](#palette) | パレット設定 |
| [pget](#pget) | 1dotの点を取得 |
| [picload](#picload) | 画像ファイルをロード |
| [pos](#pos) | カレントポジション設定 |
| [pset](#pset) | 1dotの点を表示 |
| [redraw](#redraw) | 再描画の設定 |
| [screen](#screen) | ウィンドウを初期化 |
| [width](#width) | ウィンドウサイズ設定 |
| [sysfont](#sysfont) | システムフォント選択 |
| [line](#line) | 直線を描画 |
| [circle](#circle) | 円を描画する |
| [syscolor](#syscolor) | システムカラーを設定する |
| [hsvcolor](#hsvcolor) | HSV形式でカラーを設定する |
| [rgbcolor](#rgbcolor) | RGB形式でカラーを設定する |
| [ginfo](#ginfo) | ウィンドウ情報の取得 |
| [grect](#grect) | 回転する矩形で塗りつぶす |
| [grotate](#grotate) | 矩形画像を回転してコピー |
| [gsquare](#gsquare) | 任意の四角形を描画 |
| [objinfo](#objinfo) | ウィンドウオブジェクト情報の取得 |
| [axobj](#axobj) | ActiveXコントロールの配置 |
| [winobj](#winobj) | ウィンドウオブジェクトの配置 |
| [sendmsg](#sendmsg) | ウィンドウメッセージの送信 |
| [groll](#groll) | ウィンドウの描画基点とスケールを設定 |
| [gradf](#gradf) | 矩形をグラデーションで塗りつぶす |
| [celload](#celload) | 画像ファイルをバッファにロード |
| [celdiv](#celdiv) | 画像素材の分割サイズを設定 |
| [celput](#celput) | 画像素材を描画 |
...
**関連**: print, font, pos, objcolor, %port+, Let
...
**関連**: mes, font, pos, objcolor, %port+, Let
...
**動作条件**: HSPLetアプレットの場合はステータスバーのテキストが変化します。
...
**関連**: screen, buffer, groll, screen_hide, screen_normal, screen_palette, %port+, Let
...
**動作条件**: HSPLet使用時は、パレットモードを使用できません。
...
**関連**: color, hsvcolor, rgbcolor, palcolor, line, circle, gradf, %port+, Let
...
**関連**: screen, bgscr, screen_normal, screen_palette, %port+, Let
...
**関連**: palcolor, hsvcolor, rgbcolor, %port+, Let
...
**関連**: sysfont, objmode, font_antialias, font_bold, font_italic, font_normal, font_strikeout, font_underline, %port+, Let
...
**関連**: gcopy, gmode_add, gmode_alpha, gmode_gdi, gmode_mem, gmode_pixela, gmode_rgb0, gmode_rgb0alpha, gmode_sub, %port+, Let

**動作条件**: HSPLet時、モード7はサポートしていません。
...
**動作条件**: HSPLet時、モード2はサポートされていません、またモード1で必ずアクティブになるとは限りません。
...
**関連**: color, hsvcolor, rgbcolor, palette
...
**関連**: ginfo, ginfo_r, ginfo_g, ginfo_b, %port+, Let
...
**関連**: screen, buffer, bgscr, imgload, %port+, Let

**動作条件**: HSPLet時には、BMP、JPEG、GIF、PNGの読み込みをサポートしています。その他の読み込みはサポートしていません。
...
**関連**: pget, color, palcolor, hsvcolor, %port+, Let
...
**関連**: bgscr, buffer, groll, screen_fixedsize, screen_frame, screen_hide, screen_normal, screen_palette, screen_tool, %port+, Let

**動作条件**: HSPLet時、パレットモード・ツールウィンドウ・深い淵のあるウィンドウは使用できません。
...
**関連**: screen, buffer, bgscr, %port+, Let
...
**関連**: pos, color, palcolor, boxf, circle, %port+, Let
...
**関連**: boxf, line, color, palcolor, hsvcolor, %port+, Let
...
**関連**: color, palcolor, hsvcolor, %port+, Let
...
**動作条件**: HSPLet時、タイプ 19、画面のカラーモードは常に 0 を返します。
...
**関連**: color, hsvcolor, palcolor, grotate, gsquare, gmode, %port+, Let
...
**関連**: grect, gsquare, gmode, %port+, Let
...
**関連**: grect, grotate, gmode, color, hsvcolor, palcolor, gradf, %port+, Let
...
**関連**: ginfo, objinfo_bmscr, objinfo_hwnd, objinfo_mode, %port+, Let

**動作条件**: HSPLet時、ウィンドウハンドル（type==2）のみが利用可能です。
...
**関連**: newcom, delcom, querycom

**動作条件**: ActiveXコントロールの配置には、「atl.dll」を使用します。
...
**関連**: hwnd, objinfo, winobj, %port+, Let

**動作条件**: HSPLet時、以下のメッセージのみ実装済みです。 
0x00B0:EM_GETSEL 
0x00B1:EM_SETSEL 
0x00C6:EM_CANUNDO 
0x00C7:EM_UNDO 
...
**関連**: picload, buffer, celdiv, celput
...
**関連**: celload, celdiv, gcopy, grotate
