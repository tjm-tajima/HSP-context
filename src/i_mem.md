...
| [alloc](#alloc) | バッファを確保 |
| [dim](#dim) | 配列変数を作成 |
| [dimtype](#dimtype) | 指定タイプの配列変数を作成 |
| [poke](#poke) | バッファに1byte書き込み |
| [wpoke](#wpoke) | バッファに2byte書き込み |
| [lpoke](#lpoke) | バッファに4byte書き込み |
| [sdim](#sdim) | 文字列型配列変数を作成 |
| [ddim](#ddim) | 実数型配列変数を作成 |
| [memcpy](#memcpy) | メモリブロックのコピー |
| [memset](#memset) | メモリブロックのクリア |
| [dup](#dup) | クローン変数を作成 |
| [dupptr](#dupptr) | ポインタからクローン変数を作成 |
| [mref](#mref) | 特殊なメモリを変数に割り当てる |
| [newmod](#newmod) | モジュール型変数の作成 |
| [delmod](#delmod) | モジュール型変数の要素削除 |
| [memexpand](#memexpand) | メモリブロックの再確保 |
| [ldim](#ldim) | ラベル型配列変数を作成 |
| [newlab](#newlab) | ラベル型変数を初期化 |
...
**関連**: sdim, ddim, ldim, dimtype, %port+, Let
...
**関連**: dim, sdim, ddim, ldim, vartype, %port+, Let
...
**関連**: dim, ddim, ldim, dimtype, %port+, Let
...
**関連**: dim, sdim, ldim, dimtype, %port+, Let
...
**関連**: dup, dupptr, #deffunc, %port+, Let

**動作条件**: HSPLet時、stat/refstr のみ使用可能です。
...
**関連**: #modfunc, #modinit, #modterm, delmod, foreach
