...
| 命令 | 概要 |
|---|---|
| [#define](#define) | 新規マクロを登録する |
| [#func](#func) | 外部DLL呼び出し命令登録 |
| [#cfunc](#cfunc) | 外部DLL呼び出し関数登録 |
| [#include](#include) | 別ファイルを結合 |
| [#addition](#addition) | 別ファイルを結合2 |
| [#uselib](#uselib) | 外部DLLの指定 |
| [#global](#global) | モジュールの終了 |
| [#module](#module) | モジュールの開始 |
| [#deffunc](#deffunc) | 新規命令を登録する |
| [#defcfunc](#defcfunc) | 新規関数を登録する |
| [#pack](#pack) | PACKFILE追加ファイル指定 |
| [#epack](#epack) | PACKFILE暗号化ファイル指定 |
| [#packopt](#packopt) | 自動作成オプション指定 |
| [#const](#const) | マクロ名の定数定義 |
| [#undef](#undef) | マクロ名の取り消し |
| [#if](#if) | 数値からコンパイル制御 |
| [#ifdef](#ifdef) | マクロ定義からコンパイル制御 |
| [#ifndef](#ifndef) | マクロ定義からコンパイル制御2 |
| [#else](#else) | コンパイル制御を反転 |
| [#endif](#endif) | コンパイル制御ブロック終了 |
| [#modfunc](#modfunc) | 新規命令を割り当てる |
| [#modcfunc](#modcfunc) | 新規関数を割り当てる |
| [#modinit](#modinit) | モジュール初期化処理の登録 |
| [#modterm](#modterm) | モジュール解放処理の登録 |
| [#regcmd](#regcmd) | 拡張プラグインの登録 |
| [#cmd](#cmd) | 拡張キーワードの登録 |
| [#usecom](#usecom) | 外部COMインターフェースの指定 |
| [#comfunc](#comfunc) | 外部COM呼び出し命令登録 |
| [#enum](#enum) | マクロ名の定数を列挙 |
| [#runtime](#runtime) | ランタイムファイルの設定 |
| [#cmpopt](#cmpopt) | コンパイル時の設定 |
| [#bootopt](#bootopt) | ランタイム起動時の設定 |
| [#aht](#aht) | AHTファイルヘッダを記述 |
| [#ahtmes](#ahtmes) | AHTメッセージの出力 |
...
**関連**: #const, #ifdef, #ifndef, %port+, Let
...
**関連**: #uselib, #cfunc, %port+, Let

**動作条件**: HSPLet時は、関数と同名のメソッドが呼ばれることになります。(詳細は、HSPLetマニュアルを参照してください。)
...
**動作条件**: HSPLet時は、指定された DLL と同名のクラスが読み込まれます。
...
**関連**: #global, #deffunc, #modfunc, #modcfunc, #modinit, newmod, delmod, %port+, Let
...
**関連**: #global, #module, #modfunc, #modcfunc, #modinit, #modterm, mref, %port+, Let

**動作条件**: HSPLet時、引数の型は str/int/double/var/array のみサポートされています。
local などは指定できません。 
...
**関連**: #else, #endif, #ifdef, #ifndef, #define, #const, %port+, Let
...
**関連**: #if, #ifndef, #else, #endif, %port+, Let
...
**関連**: #if, #ifdef, #else, #endif, %port+, Let
...
**関連**: #if, #ifdef, #ifndef, #endif, %port+, Let
...
**関連**: #if, #ifdef, #ifndef, #else, %port+, Let
...
**関連**: #modcfunc, #deffunc, #modinit, #modterm, thismod
...
**関連**: #modfunc, #deffunc, #modinit, #modterm, thismod
...
**関連**: #deffunc, #modfunc, #modterm, newmod
...
**関連**: #deffunc, #modfunc, #modinit, delmod
...
**関連**: #comfunc, newcom, delcom, querycom
...
**関連**: #usecom, newcom, delcom, querycom
...
**関連**: #packopt, #cmpopt, %port+, Let

**動作条件**: HSPLet使用時は、"hsplet3"をランタイム名に指定してください。
